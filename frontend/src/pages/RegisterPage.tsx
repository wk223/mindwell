    ...
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden" style={{ background: "var(--bg-deep)" }}>
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-sm" style={{ color: "var(--text-secondary)" }}>开启你的心灵疗愈之旅</p>
        </div>

        <div className="glass-heavy rounded-3xl p-8">
          <h2 className="font-serif text-xl font-medium mb-6" style={{ color: "var(--text-primary)" }}>注册</h2>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="nickname" className="block text-sm font-medium mb-1.5" style={{ color: "var(--text-secondary)" }}>
                昵称
              </label>
              <input
                id="nickname"
                ...
                style={{ background: "var(--bg-glass)", border: "0.5px solid var(--border-light)", color: "var(--text-primary)" }}
              />
            </div>
            <div>
          ...
          <p className="text-center text-sm mt-6" style={{ color: "var(--text-tertiary)" }}>
            已有账号？{" "}
            <Link to="/login" className="font-medium transition-colors" style={{ color: "var(--accent-400)" }}>
              登录
            </Link>
          </p>
        </div>
