...
import Echo from "../components/night/Echo";
...
const TABS = [
  { key: "echo", label: "ECHO", tip: "答案之书" },
  { key: "chat", label: "AI倾诉", tip: "深夜陪伴" },
];
  ...
  return (
    <div className="h-full relative overflow-hidden" style={{ background: "#020617" }}>
      ...
      <div
        ...
        style={{
          background: "radial-gradient(ellipse at 50% 30%, rgba(56,189,248,0.06) 0%, transparent 60%), radial-gradient(ellipse at 50% 80%, rgba(99,102,241,0.04) 0%, transparent 50%)",
        }}
      />

      {/* Particles */}
      <Particles active={activeTab === "echo"} />
      ...
      <div className="relative z-10 h-full flex flex-col">
        ...
        <div className="flex items-center pt-4 sm:pt-6 pb-2 px-4">
          ...
          {TABS.map((tab) => (
            <button
              ...
              className={cn(
                "relative px-4 sm:px-6 py-2.5 rounded-2xl text-sm sm:text-base tracking-wider transition-all duration-500",
                activeTab === tab.key
            ...
            >
              {tab.label}
              <span className="hidden sm:inline text-slate-500 text-xs ml-1.5">· {tab.tip}</span>
              {activeTab === tab.key && (
                <span className="absolute bottom-1 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-gradient-to-r from-sky-400 to-indigo-400 rounded-full" />
              )}
            </button>
          ))}
          </div>
        ...
        <div className="text-center pb-4 sm:pb-6 pt-2 px-4 space-y-2">
          ...
          <p className="text-[10px] sm:text-xs text-slate-500/60 tracking-widest">
            ECHO · MindWell
          </p>
